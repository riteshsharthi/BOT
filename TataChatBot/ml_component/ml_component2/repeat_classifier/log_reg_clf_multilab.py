...
base_path_data = os.path.join(base_dir, 'repeat_classifier', 'data')

"""
Class to perform pickle operations
"""
class LogisticClassifierRepeatTask():
    def __init__(self):
    ...
    """
    def test(self, query):
        modifier_dict = {"repeat_data.text":"repeat_query",
...
if __name__ == '__main__':
        while(True):
            qr = input("=>")
            result = LogisticClassifierRepeatTask()
            d = result.test(qr)
            print (d)
