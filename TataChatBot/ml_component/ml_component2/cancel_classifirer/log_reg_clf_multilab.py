...
base_path_data = os.path.join(base_dir, 'cancel_classifirer', 'data')

"""
Class to perform pickle operations
"""
class LogisticClassifierCancelTask():
    def __init__(self):
    ...
    def getData(self):
        ...
        try:
            for ifile in self.labels:
                with open(os.path.join(train_path,ifile),"r") as tf:
    ...
    """
    def test(self, query):
        modifier_dict = {"cancel_task.text":"cancel_query",
                         "proceeding_task.text":"proceeding_query"
...
if __name__ == '__main__':
        while(True):
            qr = input("=>")
            result = LogisticClassifierCancelTask()
            d = result.test(qr)
            print (d)
